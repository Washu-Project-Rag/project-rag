import os
import re
import json
import glob
import hashlib
from typing import List, Dict, Iterable, Tuple, Optional

from tqdm import tqdm


# -----------------------
# 1) Computer-science-focused topic filter
# -----------------------
TECH_KEYWORDS = [
    # core CS
    "algorithm", "data structure", "computer science", "computing", "software",
    "programming", "programming language", "api",

    # systems
    "operating system", "kernel", "compiler", "interpreter", "runtime",
    "thread", "process", "concurrency", "deadlock", "memory management",
    "virtual memory", "file system", "cache",

    # distributed / cloud
    "distributed system", "distributed computing", "cloud computing",
    "container", "kubernetes", "docker", "load balancer",

    # networking
    "network", "protocol", "tcp", "udp", "ip", "http", "tls", "ssl",
    "dns", "routing", "packet",

    # security
    "encryption", "cryptography", "cipher", "authentication",
    "authorization", "public-key", "private-key",

    # data / DB
    "database", "sql", "index", "query", "transaction", "normalization",
    "replication",

    # algorithms / DS
    "binary search", "binary search tree", "hash table", "queue", "stack",
    "graph", "tree",

    # popular PLs
    "python", "java", "c++", "javascript",
]

TECH_REGEX = re.compile(
    r"\b(" + "|".join(re.escape(k) for k in TECH_KEYWORDS) + r")\b",
    re.IGNORECASE
)

NEGATIVE_KEYWORDS = [
    # obvious non-CS
    "railway station", "football", "basketball", "album", "song",
    "film", "actress", "actor", "politician", "school", "village",
    "river", "mountain", "species", "genus", "taxon",
    "church", "municipality", "district", "cricketer", "singer",

    # politics / government
    "agreement", "treaty", "constitution", "election", "political party",
    "parliament", "government", "president", "prime minister",

    # entertainment / media
    "tv network", "television", "channel", "episode", "series",
    "band", "music", "record label", "filmography",
]

NEG_REGEX = re.compile(
    r"\b(" + "|".join(re.escape(k) for k in NEGATIVE_KEYWORDS) + r")\b",
    re.IGNORECASE
)

# Traditional wiki heading format
HEADING_RE = re.compile(r"^(={2,6})\s*(.+?)\s*\1\s*$")

# Heuristic top-level headings commonly seen in WikiExtractor-cleaned biography/article text
TOP_LEVEL_HEADINGS = {
    "History", "Overview", "Background", "Career", "Club career", "Professional career",
    "Academic career", "Political career", "Military career", "Business career",
    "International career", "Life", "Personal life", "Early life", "Education",
    "Outside football", "Post-retirement", "Legacy", "Reception", "Style of play",
    "Works", "Bibliography", "Discography", "Filmography", "Honours", "Honors",
    "Awards", "See also", "References", "External links", "Publications",
    "Gameplay", "Development", "Plot", "Premise", "Applications", "Examples",
    "Representation", "Analysis", "Properties", "Cryptographic analysis",
    "Design", "Operation", "Architecture", "Implementation"
}

# Strong signals that a short standalone line is probably NOT a heading
SENTENCE_LIKE_RE = re.compile(r"[,:;!?]")


def is_technical_article(title: str, text: str, min_hits: int = 4) -> bool:
    """
    Simple CS-focused filter using title + first part of text.
    """
    title_l = (title or "").lower()

    if any(x in title_l for x in [
        "agreement", "treaty", "tv network", "television", "album",
        "song", "film", "footballer", "railway station"
    ]):
        return False

    blob = (title or "") + "\n" + (text or "")[:4000]

    if NEG_REGEX.search(blob):
        return False

    hits = len(TECH_REGEX.findall(blob))
    return hits >= min_hits


def stable_chunk_id(page_id: str, title: str, section_path: List[str], para_idx: int, sub_idx: int) -> str:
    """
    Stable deterministic chunk ID.
    """
    base = f"{page_id}||{title}||{' > '.join(section_path)}||{para_idx}||{sub_idx}"
    h = hashlib.md5(base.encode("utf-8")).hexdigest()[:12]
    return f"wiki_{h}"


def iter_articles_from_wikiextractor_files(paths: List[str]) -> Iterable[Dict]:
    """
    WikiExtractor --json output: one JSON object per line.
    Typical fields: id, revid, url, title, text
    """
    for p in paths:
        with open(p, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
                try:
                    obj = json.loads(line)
                    yield obj
                except json.JSONDecodeError:
                    continue


def normalize_heading(line: str) -> str:
    """
    Normalize heading text:
    - strip whitespace
    - remove trailing period
    """
    line = line.strip()
    if line.endswith("."):
        line = line[:-1].strip()
    return line


def is_short_heading_line(line: str) -> bool:
    """
    Heuristic for WikiExtractor-cleaned heading lines like:
      Club career.
      Sporting.
      Barcelona.
      International career.
      Personal life.

    We only want short, standalone lines that don't look like full sentences.
    """
    s = line.strip()
    if not s:
        return False

    # Very long lines are almost certainly not headings
    if len(s) > 80:
        return False

    # Too many words -> likely a sentence, not a heading
    if len(s.split()) > 8:
        return False

    # Contains obvious sentence punctuation other than final period
    if SENTENCE_LIKE_RE.search(s):
        return False

    # If it ends with a period and is short, likely a heading in your data
    if s.endswith("."):
        return True

    # Also allow short title-case lines without final period, e.g. "Benfica"
    if len(s.split()) <= 4 and s[0].isupper():
        return True

    return False


def infer_heading_level(heading: str, current_top: Optional[str]) -> int:
    """
    Infer heading level from cleaned text.

    Returns:
      1 -> top-level section
      2 -> second-level subsection
    """
    if heading in TOP_LEVEL_HEADINGS:
        return 1

    # If we already have a current top-level section, short specific headings are likely subheadings
    if current_top is not None:
        return 2

    # If no current top-level section exists yet, treat as top-level
    return 1


def split_long_paragraph(para: str, max_chars: int = 1200) -> List[str]:
    """
    Character-based split for long paragraphs.
    """
    para = para.strip()
    if len(para) <= max_chars:
        return [para]

    chunks = []
    start = 0
    while start < len(para):
        end = min(len(para), start + max_chars)

        cut = max(
            para.rfind(". ", start, end),
            para.rfind("\n", start, end)
        )

        if cut <= start + 200:
            cut = end
        else:
            cut += 1

        chunk = para[start:cut].strip()
        if chunk:
            chunks.append(chunk)
        start = cut

    return chunks


def parse_sections_and_paragraphs(text: str) -> Iterable[Tuple[List[str], int, str]]:
    """
    Parse article text into:
      (section_path, paragraph_index, paragraph_text)

    Supports BOTH:
    1) explicit wiki headings, e.g. == Heading ==
    2) WikiExtractor-cleaned short standalone heading lines, e.g.:
         Club career.
         Sporting.
         Barcelona.

    Assumptions for your current data:
    - Each newline-separated non-heading line is typically a paragraph
    - Intro text before any heading belongs to root
    """
    lines = (text or "").splitlines()

    current_top: Optional[str] = None
    current_section_path: List[str] = []
    paragraph_index = 0

    def emit_paragraph(section_path: List[str], paragraph_text: str):
        nonlocal paragraph_index
        para = paragraph_text.strip()
        if para and len(para) >= 40:
            yield (section_path.copy(), paragraph_index, para)
            paragraph_index += 1

    for raw_ln in lines:
        ln = raw_ln.strip()
        if not ln:
            continue

        # Case 1: explicit wiki heading == Heading ==
        m = HEADING_RE.match(ln)
        if m:
            level = len(m.group(1))
            heading = m.group(2).strip()

            if level <= 2:
                current_top = heading
                current_section_path = [heading]
            else:
                if current_top is None:
                    current_top = heading
                    current_section_path = [heading]
                else:
                    current_section_path = [current_top, heading]
            continue

        # Case 2: cleaned heading line like "Club career." or "Sporting."
        if is_short_heading_line(ln):
            heading = normalize_heading(ln)
            level = infer_heading_level(heading, current_top)

            if level == 1:
                current_top = heading
                current_section_path = [heading]
            else:
                if current_top is None:
                    current_top = heading
                    current_section_path = [heading]
                else:
                    current_section_path = [current_top, heading]
            continue

        # Case 3: normal paragraph line
        yield from emit_paragraph(current_section_path, ln)


def main(
    input_glob: str = "data/raw/wiki_extracted_sample/*",
    output_path: str = "data/processed/corpus_chunks.jsonl",
    source_name: str = "enwiki_pages_articles_latest",
    min_hits: int = 4,
    max_articles: Optional[int] = 2000,
):
    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    paths = sorted(glob.glob(input_glob))
    if not paths:
        raise FileNotFoundError(f"No files matched: {input_glob}")

    kept_articles = 0
    kept_chunks = 0

    with open(output_path, "w", encoding="utf-8") as out_f:
        for art in tqdm(iter_articles_from_wikiextractor_files(paths), desc="articles"):
            page_id = str(art.get("id", "") or "")
            revid = str(art.get("revid", "") or "")
            url = art.get("url", "") or ""
            title = art.get("title", "") or ""
            text = art.get("text", "") or ""

            if not is_technical_article(title, text, min_hits=min_hits):
                continue

            kept_articles += 1

            for section_path, para_idx, para in parse_sections_and_paragraphs(text):
                if not section_path:
                    section_path = ["(root)"]

                sub_chunks = split_long_paragraph(para, max_chars=1200)

                for sub_idx, sub in enumerate(sub_chunks):
                    chunk_id = stable_chunk_id(page_id, title, section_path, para_idx, sub_idx)

                    rec = {
                        "chunk_id": chunk_id,
                        "page_id": page_id,
                        "title": title,
                        "section_path": section_path,
                        "paragraph_index": para_idx,
                        "text": sub,
                        "source": source_name,
                        "url": url,
                        "revid": revid,
                    }

                    out_f.write(json.dumps(rec, ensure_ascii=False) + "\n")
                    kept_chunks += 1

            if max_articles is not None and kept_articles >= max_articles:
                break

    print(f"Done. kept_articles={kept_articles}, kept_chunks={kept_chunks}")
    print(f"Wrote: {output_path}")


if __name__ == "__main__":
    main()